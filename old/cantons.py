...
with open('donnees_cantons.csv', newline='', encoding='utf-8') as csvfile:
    spamreader = csv.reader(csvfile, delimiter=';')
    for row in spamreader:
        if row[0].isnumeric():
            code_insee = row[2] + row[3]
            if int(code_insee.replace('A', '0').replace('B', '0')) > skip_to_insee:
                population = int(row[6])  # PMUN
                ...
                id_items = frc.get_items(claims=[ExternalID(prop_nr='P2506', value=str(code_insee))])
                ...
                if id_item:
                    ...
                    if write_needed:
                        print(f'Write to Wikidata for {row[4]} ({row[2]})')
                        try:
                            wb_item.write(summary='Update population for 2019')
                        except MWApiError as e:
                            print(e)
                    else:
                        print(f'Skipping {row[4]} ({row[3]})')
