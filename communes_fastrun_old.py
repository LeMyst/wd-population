...
frc = wbi_fastrun.get_fastrun_container(base_filter=base_filter, use_qualifiers=False, use_references=False, use_rank=False, cache=True)
...
with open('annees/' + config.year + '/donnees_communes.csv', newline='', encoding='utf-8') as csvfile:
    ...
    for row in spamreader:
        ...
        if row[0].isnumeric():
            ...
            logging.debug(code_insee)

            if not already_skipped and code_insee != skip_to_insee:
                continue
